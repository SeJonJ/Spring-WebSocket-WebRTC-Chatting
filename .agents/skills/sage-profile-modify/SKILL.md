...
> `docs/sage_harness/skills/sage-profile-modify.md`. Codex reads this render from the
> repo (`.Codex/skills/sage-profile-modify/`); Codex reads it from the user-global
...
**`team.core.<role>.runtime.{model,effort}`** are injected into `.Codex/agents/<id>.md`
...
`stale`. They bind on a Codex host only. A bare `model:`/`effort:` directly on the role
...
`Codex`: `low|medium|high|xhigh|max`) and a wrong value is rejected by `sage validate`.
