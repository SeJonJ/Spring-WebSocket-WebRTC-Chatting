...
- `options.cross_model: true` AND the peer runtime CLI is available
  (Codex-host→`codex`, codex-host→`Codex`) → **opposite-runtime review** (cross-model, removes model bias).
  SAGE invokes the peer directly — Codex-host runs `codex exec`, codex-host runs `Codex -p`
...
`sage cross-check --packet-file <f>`; it invokes the peer (`codex exec`/`Codex -p`) and prints the
...
> template, not via `sage generate`. Deploy location is runtime-specific: Codex
> reads it from the repo (`.Codex/skills/sage-review/`); Codex reads it from the
