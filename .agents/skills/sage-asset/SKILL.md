---
name: sage-asset
description: "Add or modify a SAGE governance asset (hook / agent / skill) through conversation — interview intent, author the spec and renders (BOTH Codex and codex), then run sage generate to extract+register. Invoke when the user says /sage-asset, add an agent/skill/hook, modify an asset, 자산 추가, 에이전트/스킬/훅 추가·수정, or after bootstrap to extend the team/roster."
---
...
> `docs/sage_harness/skills/sage-asset.md`. Codex reads this render from the repo
> (`.Codex/skills/sage-asset/`); Codex reads it from the user-global skills dir
> (`$CODEX_HOME/skills/sage-asset/`).

## Hard rules

- **Never edit generated artifacts to "change" an asset.** The write-guard blocks
  direct edits of `.Codex/.codex/{agents,hooks,skills}` (except CORE bootstrap
  renders). To change an asset: edit its spec (hook) or render (agent/skill) source,
  then re-run `sage generate`.
- **codex 함께 — author BOTH runtimes.** Every agent/skill gets a Codex render AND
...
2. Hand off: `sage generate --kind hook --write --target <Codex|codex|both>`
   (registers settings.json/hooks.json + manifest stamp).

### agent / skill (interpretive — author BOTH renders)
1. Author the **Codex render**:
   - agent → `.Codex/agents/<id>.md` (frontmatter `name`+`description` + body)
...
in the manifest (render_hash for both Codex and codex). It **fails closed** if either
