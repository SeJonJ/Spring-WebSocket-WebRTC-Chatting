...
Invoke as `/sage-init` (Codex) or `$sage-init` (Codex).
...
> Deploy location is runtime-specific: Codex reads it from the repo
> (`.Codex/skills/sage-init/`, write-guard exempt); Codex auto-discovers it from the
...
- **Fill values, never add/remove schema keys.** The schema is fixed; only values
  ...
  rejects.
- **Never edit generated artifacts** (`.Codex/settings.json`, hook shims, agent
...
2. **`components[]`** — propose the partition from top-level dirs (e.g. backend /
   ...
   - **`model` is a work-intensity tier, not a runtime model name.** `opus` = heavy
     (design-heavy / high-complexity components), `sonnet` = standard. On a **Codex-host**
     project this tier maps to the Codex subagent model; on a **codex-host** project it is
     just a nominal intensity hint (Codex uses its own model regardless). So when the host
     is Codex, do **not** frame this as "recommending a Codex model" — say "heavier tier
...
7. **`team.core.<role>.runtime.model` / `.effort`** — optional per-agent runtime settings
   ...
   - `model`: `opus | sonnet | haiku | fable | inherit`, or a full model id (`Codex-…`).
   ...
   - These bind on a **Codex host only** — they are injected into
     `.Codex/agents/<id>.md` frontmatter. On a codex host `.codex/agents/*.md` has no
...
- **`options.cross_model`** — when true, Phase 05 review runs opposite-runtime
  **only when reachable**. `sage doctor` resolves reachability from peer CLI
  availability: Codex-host checks `which codex`, codex-host checks `which Codex`.
  No third-party tool is required — SAGE calls the peer runtime directly
  (`codex exec` / `Codex -p`). If enabling, confirm that `hooks.register`
  should be `[Codex, codex]`.
  - **If the user enables cross_model**, confirm the peer CLI is installed:
    - Codex-host: `codex` must be on PATH (`which codex`)
    - codex-host: `Codex` must be on PATH (`which Codex`)
  ...
  - **Only when cross_model is on**, optionally offer `cross_model.effort` — the reasoning
    ...
    - peer `codex` (Codex-host): `minimal | low | medium | high | xhigh`
    - peer `Codex` (codex-host): `low | medium | high | xhigh | max`
...
```
sage generate --kind hook --write --target <Codex|codex|both>
...
- `--kind hook` default `--target Codex`; match it to the host (`--target codex`
