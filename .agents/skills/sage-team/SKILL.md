...
- **Codex host**: spawn implementers as **parallel subagents** (one per component).
...
2. **Match the vault's own authoring guide, then run.** Before writing, check the vault
   root for an authoring guide — first found of `AGENTS.md` / `AGENTS.md` / `GEMINI.md` /
...
- **A note path printed** → it is written **empty on purpose**. `retro` gathers the evidence
  ...
  Stop hook records the cycle as unfinished and (under `enforce`, Codex host) blocks stopping once.
...
> not via `sage generate`. Deploy location is runtime-specific: Codex reads it from the
> repo (`.Codex/skills/sage-team/`); Codex reads it from the user-global skills dir
